...
zsh-vi-mode can integrate with your system clipboard. This feature is disabled by default.
...
- **macOS**: `open`
- **Linux**: `xdg-open`
- **Windows**: `start` (Command Prompt) or `powershell.exe` (PowerShell)
...
For Windows users, you can set the `open` command as a alias as below:
...
Or more complex commands by creating a script, for example, create a script
`/usr/local/bin/xdg-open` for opening URL on Windows:
